...
- 오브젝트 AABB는 모든 부품 바운드의 합집합(교집합/차집합도 포함)이라 보수적. 실제 크기보다 커도 결과는 맞고, 성능만 조금 손해
...
| 거리 안개 | `rtKernel` 끝 — 지면 경계를 지우고 원경에 공기감 |
| 화이트포인트 있는 Reinhard 톤 매핑 | `rtKernel` 끝 |
| 바운스 소진 시 하늘로 근사 | `rtKernel` 끝 — 없으면 유리를 여러 겹 통과하는 픽셀이 까맣게 죽는다 |
...
- 부품 추가: 폴리곤 압출(`candPlanes`에 평면 n개), 타원체(구를 비균등 스케일하면 이미 됨), 회전체
- 재질: 반사/굴절 바운스 (구간 정보가 있으니 굴절은 특히 쉬움)
